...
if 0 == 0 (
  ...
  set "JB_COMMAND_SUFFIX=-jar launcher\lib\launcher.jar Main"
  setlocal enabledelayedexpansion
)
...
if not "!PREP_SCRIPT!" == "" (
  call !PREP_SCRIPT! !JB_ALL_ARGS! <nul
  set PREP_SCRIPT_EXIT_CODE=!ERRORLEVEL!
  if not "!PREP_SCRIPT_EXIT_CODE!" == "0" (
    echo !APP_DISPLAY_NAME! prep script failed: exit code !PREP_SCRIPT_EXIT_CODE! 1>&2
    echo Prep script: !PREP_SCRIPT! !JB_ALL_ARGS! 1>&2
    exit /b -1
  )
)
