...
# --------------------------------------------------------------------------------------
version_ge() {
  major1=`echo "$1" | awk -F. '{print($1)}' 2>/dev/null`
  minor1=`echo "$1" | awk -F. '{print($2)}' 2>/dev/null`
  major2=`echo "$2" | awk -F. '{print($1)}' 2>/dev/null`
  minor2=`echo "$2" | awk -F. '{print($2)}' 2>/dev/null`
...
check_java_internal() {
  ...
  if [ $? -ne 0 ]; then
    if [ -n "$FJ_DEBUG" ]; then echo "[`date +%T`] $FJ_DEBUG_INDENT    Version is not suitable" 1>&2; fi
    return 1
  fi
...
find_java() {
  ...
  if [ -n "$FJ_DEBUG" ]; then
    ...
    if [ -n "$FJ_LOOK_FOR_SERVER_JAVA$FJ_LOOK_FOR_X64_JAVA" ]; then echo "" 1>&2; fi
  fi
