...
if "%~1" == "fun" (
  ...
  if "!FUN_NAME!" == "version_ge" (
    for /f "tokens=1,2 delims=." %%i in ("!FUN_ARG1!") do (
      for /f "tokens=1,2 delims=." %%x in ("!FUN_ARG2!") do (
        endlocal
        set "II=%%i"
        set "JJ=%%j"
        set "XX=%%x"
  ...
  if "!FUN_NAME!" == "check_java_internal" (
    ...
    if not "!ERRORLEVEL!" == "0" (
      if not "!FJ_DEBUG!" == "" echo [!TIME!] !FJ_DEBUG_INDENT!    Version is not suitable 1>&2
      goto exit_fail
    )
...
if not "!FJ_DEBUG!" == "" (
  ...
  if not "!FJ_LOOK_FOR_SERVER_JAVA!!FJ_LOOK_FOR_X64_JAVA!" == "" echo. 1>&2
)
